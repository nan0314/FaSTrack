...
int main(int argc, char **argv)
{
    ...
    vector<double> gMin;
    vector<double> gMax;
    vector<double> deriv3;
    vector<double> deriv4;
    vector<double> pMax;
    ...
    n.getParam("deriv3",deriv3);
    n.getParam("deriv4",deriv4);
    ...
    while (ros::ok())
    {
        ...
        int r_index = r0*gN[1]*gN[2]*gN[3]*gN[4] + r1*gN[2]*gN[3]*gN[4] + r2*gN[3]*gN[4] + r3*gN[4] + r4;       // row major indexing
        // int r_index = r4*gN[3]*gN[2]*gN[1]*gN[0] + r3*gN[2]*gN[1]*gN[0] + r2*gN[1]*gN[0] + r1*gN[0] + r0;    // col major indexing

        if (deriv3.at(r_index) >= 0){
            control_msg.data[0] = aRange[0];
        } else {
            control_msg.data[0] = aRange[1];
        }

        if (deriv4.at(r_index) >= 0){
        ...
        std::cout << control_msg.data[0] << " " << control_msg.data[1] << std::endl;
